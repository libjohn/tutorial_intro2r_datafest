...
Additionally, I'm using a **pipe** operator (`|>`) as a conjunction to connect functions. In this way I can form a *data sentence*. Many people call the data sentence a *data pipe*, or just a *pipe*. Another common pipe operator is that may see looks like this: `%>%`. `\>` and `%>%` are basically synonymous.
...
Let' make a quick line plot showing how many people participate in races each year. Here we have a `data` field this is also a date data-type. Data types are important and in this example using a data data-type means {`ggplot2`} will simplify our x-axis labels.
